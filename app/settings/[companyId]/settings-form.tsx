...
export function SettingsForm({
	...
	return (
		<div className="space-y-6">
			<div className="rounded-xl border border-white/75 dark:border-slate-600/50 bg-white/78 dark:bg-slate-800/40 px-4 py-3 shadow-[0_10px_24px_rgba(15,23,42,0.08)] dark:shadow-[0_10px_24px_rgba(0,0,0,0.2)]">
				<Text size="2" weight="medium" className="text-[#FA4616] dark:text-orange-400">
					Write like a human. Keep it short, warm, and clear.
				</Text>
				<Text size="1" color="gray" className="mt-1 dark:text-slate-400">
			...
			>
				<div className="space-y-3 rounded-lg border border-white/75 dark:border-slate-600/50 bg-white/78 dark:bg-slate-800/40 p-3 shadow-[0_8px_20px_rgba(15,23,42,0.06)] dark:shadow-[0_8px_20px_rgba(0,0,0,0.15)]">
					<div className="flex flex-wrap items-center gap-3">
						<Text
							size="1"
							weight="bold"
							className="uppercase tracking-[0.08em] text-zinc-600 dark:text-slate-400"
						>
							Send after
						</Text>
						...
						>
							<Select.Trigger className="bg-white/90 dark:bg-slate-700" />
							<Select.Content>
			...
			{isSyncing ? (
				<div className="inline-flex items-center gap-2 rounded-full border border-white/75 dark:border-slate-600/50 bg-white/80 dark:bg-slate-800/40 px-3 py-1.5 shadow-[0_8px_20px_rgba(15,23,42,0.08)] dark:shadow-[0_8px_20px_rgba(0,0,0,0.2)]">
					<Spinner loading size="1" />
					<Text color="gray" size="2" className="dark:text-slate-400">
						Syncing members...
					</Text>
				</div>
			) : (
			...
			{error ? (
				<Text size="2" color="red" className="dark:text-red-400">
					{error}
				</Text>
			) : null}
...
function TriggerCard({
	...
	return (
		<Card className="rounded-[18px] border border-white/80 dark:border-slate-600/40 bg-white/82 dark:bg-slate-800/40 p-5 shadow-[0_12px_30px_rgba(15,23,42,0.08)] dark:shadow-[0_12px_30px_rgba(0,0,0,0.2)]">
			<div className="flex items-start justify-between gap-4">
				<div className="min-w-0">
					<Heading size="4" className="leading-tight dark:text-slate-100">
						{title}
					</Heading>
					<Text size="2" color="gray" className="mt-1.5 max-w-2xl leading-6 dark:text-slate-400">
						{description}
					</Text>
				</div>
				<div className="flex flex-col items-end gap-1">
					...
					<Text
						size="1"
						weight="medium"
						className={enabled ? "text-emerald-600 dark:text-emerald-400" : "text-zinc-500 dark:text-slate-500"}
					>
						{enabled ? "Enabled" : "Paused"}
					</Text>
				</div>
			</div>
			<div className="mt-5 space-y-4 border-t border-zinc-200/80 dark:border-slate-700/50 pt-4">{children}</div>
		</Card>
	);
}
	...
	return (
		<div className="space-y-2">
			<div className="flex items-center justify-between gap-3">
				<Text size="1" weight="bold" className="uppercase tracking-[0.08em] text-zinc-600 dark:text-slate-400">
				{label}
				</Text>
			...
			<TextArea
				...
				maxLength={160}
				rows={4}
				className="leading-6 dark:bg-slate-700 dark:text-slate-100"
			/>
			<Text size="1" color="gray" className="leading-5 dark:text-slate-400">
